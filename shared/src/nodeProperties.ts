import { NodeType, PropertyDataType } from './types';
  ...
  if (Object.values(NodeType).includes(normalizedType as NodeType)) {
    switch (normalizedType as NodeType) {
      ...
      case NodeType.CSV_HANDLE:
        return [
          { name: 'action', label: 'Action', dataType: PropertyDataType.STRING, required: true, defaultValue: 'write' },
          { name: 'filePath', label: 'File path', dataType: PropertyDataType.STRING, required: true },
          { name: 'dataSource', label: 'Data source', dataType: PropertyDataType.STRING, required: false },
          { name: 'contextKey', label: 'Context key', dataType: PropertyDataType.STRING, required: false, defaultValue: 'csvData' },
          { name: 'headers', label: 'Headers', dataType: PropertyDataType.STRING, required: false },
