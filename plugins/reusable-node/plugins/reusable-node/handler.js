...
class RunReusableHandler {
    async execute(node, context) {
        ...
        const workflow = context.getData('fullWorkflow');
        if (!workflow) {
        ...
        try {
            ...
            const nodesWithNoDriverInput = reusableFlow.nodes.filter(n => {
                const hasDriverInput = reusableFlow.edges.some(e => e.target === n.id &&
        ...
        const resolvePropertyInputs = context.getData('resolvePropertyInputs');
        ...
        for (const nodeId of executionOrder) {
            ...
            const handler = (0, nodes_1.getNodeHandler)(nodeToExecute.type);
            if (!handler) {
