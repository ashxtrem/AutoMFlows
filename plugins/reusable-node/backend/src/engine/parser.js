...
const reusableFlowExtractor_1 = require("../utils/reusableFlowExtractor");
class WorkflowParser {
    constructor(workflow) {
        this.startNodeId = null;
        this.reusableScopes = null;
        ...
        // Find start node
        for (const node of workflow.nodes) {
            if (node.type === shared_1.NodeType.START) {
    ...
    getReusableScopes() {
        if (this.reusableScopes === null) {
            this.reusableScopes = (0, reusableFlowExtractor_1.getAllReusableScopes)(this.workflow);
        }
     ...
     */
    getReusableScopeForNode(nodeId) {
     ...
     */
    getExecutionOrder(excludeReusableScopes = true) {
        if (!this.startNodeId) {
        ...
        const reusableScopes = excludeReusableScopes ? this.getReusableScopes() : new Map();
        const allReusableScopeNodes = new Set();
     ...
     */
    getNodesReachableFromHandle(nodeId, sourceHandle) {
        const reachable = new Set();
        const visited = new Set();
        const visit = (currentNodeId, isInitialNode = false) => {
            ...
            if (isInitialNode) {
                ...
                outgoingEdges = this.edges.filter(edge => edge.source === currentNodeId && edge.sourceHandle === sourceHandle);
            }
            else {
                // For other nodes, follow all control flow edges (output handle or no handle)
                outgoingEdges = this.edges.filter(edge => edge.source === currentNodeId && (!edge.sourceHandle || edge.sourceHandle === 'output'));
            }
    ...
    validate() {
        ...
        for (const [nodeId, count] of driverConnections.entries()) {
            ...
            if (node && node.node.type !== shared_1.NodeType.START && count > 1) {
