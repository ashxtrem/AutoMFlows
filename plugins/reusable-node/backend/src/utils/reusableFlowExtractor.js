...
exports.findReusableByContext = findReusableByContext;
exports.getReusableScope = getReusableScope;
exports.extractReusableFlow = extractReusableFlow;
exports.getAllReusableScopes = getAllReusableScopes;
 ...
 */
function findReusableByContext(workflow, contextName) {
 ...
 */
function getReusableScope(workflow, reusableNodeId) {
    const scope = new Set();
    ...
    const traverse = (nodeId, isStart = false) => {
        ...
        const outgoingEdges = workflow.edges.filter(edge => edge.source === nodeId && (!edge.sourceHandle || edge.sourceHandle === 'output' || edge.sourceHandle === 'driver'));
 ...
 */
function extractReusableFlow(workflow, reusableNodeId) {
    const reusableNode = workflow.nodes.find(n => n.id === reusableNodeId);
    if (!reusableNode || reusableNode.type !== REUSABLE_NODE_TYPE) {
        return null;
    }
    ...
    const reusableNodes = workflow.nodes.filter(node => scope.has(node.id) && node.id !== reusableNodeId);
    ...
    const startEdges = workflow.edges.filter(edge => edge.source === reusableNodeId && (!edge.sourceHandle || edge.sourceHandle === 'output' || edge.sourceHandle === 'driver'));
 ...
 */
function getAllReusableScopes(workflow) {
