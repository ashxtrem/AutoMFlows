...
class WaitHelper {
     ...
     */
    static async executeWaits(page, options) {
        const { waitForSelector, waitForSelectorType, waitForSelectorTimeout, waitForUrl, waitForUrlTimeout, waitForCondition, waitForConditionTimeout, waitStrategy = 'parallel', failSilently = false, defaultTimeout = 30000, } = options;
        const waitPromises = [];
        const waitTiming = options.waitTiming || 'before';
        // Wait for selector
        if (waitForSelector) {
            const selectorTimeout = waitForSelectorTimeout || defaultTimeout;
            const waitForSelectorPromise = this.waitForSelector(page, waitForSelector, waitForSelectorType || 'css', selectorTimeout, failSilently, waitTiming);
            waitPromises.push(waitForSelectorPromise);
        }
        // Wait for URL pattern
        if (waitForUrl) {
            const urlTimeout = waitForUrlTimeout || defaultTimeout;
            const waitForUrlPromise = this.waitForUrl(page, waitForUrl, urlTimeout, failSilently, waitTiming);
            waitPromises.push(waitForUrlPromise);
        }
        // Wait for custom JavaScript condition
        if (waitForCondition) {
            const conditionTimeout = waitForConditionTimeout || defaultTimeout;
            const waitForConditionPromise = this.waitForCondition(page, waitForCondition, conditionTimeout, failSilently, waitTiming);
    ...
    static async waitForSelector(page, selector, selectorType, timeout, failSilently, waitTiming = 'before') {
    ...
    static async waitForUrl(page, urlPattern, timeout, failSilently, waitTiming = 'before') {
        const timingLabel = waitTiming === 'after' ? 'after operation' : 'before operation';
    ...
    static async waitForCondition(page, condition, timeout, failSilently, waitTiming = 'before') {
        const timingLabel = waitTiming === 'after' ? 'after operation' : 'before operation';
