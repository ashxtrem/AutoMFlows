...
export class LocalLLMClient {
  ...
  constructor() {
    const baseUrl = this.config.llm.local?.baseUrl || 'http://localhost:11434';
    this.httpClient = axios.create({
      baseURL: baseUrl,
      timeout: 120000, // Longer timeout for local LLMs
      headers: {
  ...
  isAvailable(): boolean {
    return this.config.llm.provider === 'local';
  }

  async generateWorkflow(request: WorkflowGenerationRequest): Promise<Workflow> {
    if (!this.isAvailable()) {
      throw new Error('Local LLM is not configured. Set LLM_PROVIDER=local environment variable.');
    }
    ...
    try {
      ...
      const response = await this.httpClient.post('/api/generate', {
        model: this.config.llm.local!.model,
        prompt: `${systemPrompt}\n\n${userPrompt}`,
        stream: false,
        format: 'json',
      });
      ...
      if (response.data.response) {
        // Ollama format
        workflowJson = JSON.parse(response.data.response);
      } else if (response.data.choices && response.data.choices[0]?.message?.content) {
        // OpenAI-compatible format
        workflowJson = JSON.parse(response.data.choices[0].message.content);
      } else {
      ...
      try {
        ...
        throw new Error(`Failed to generate workflow: ${error.message}. Fallback also failed: ${fallbackError.message}`);
      }
    }
  }
