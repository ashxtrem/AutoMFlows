                ...
                <>
                  ...
                  {data.retryUntilCondition?.type === 'api-status' && (
                    <div>
                      ...
                      {(() => {
                        const valueStr = typeof data.retryUntilCondition?.expectedStatus === 'string' 
                          ? data.retryUntilCondition.expectedStatus 
                          : (data.retryUntilCondition?.expectedStatus?.toString() ?? '');
                        const containsInterpolation = typeof data.retryUntilCondition?.expectedStatus === 'string' && valueStr.includes('${');
                        return (
                          <>
                            <input
                              type={containsInterpolation ? "text" : "number"}
                              ...
                              onChange={(e) => {
                                ...
                                if (inputValue.includes('${')) {
                                  ...
                                  onChange('retryUntilCondition', {
                                    ...data.retryUntilCondition,
                                    expectedStatus: isNaN(numValue) ? 200 : numValue,
                                  });
                                }
                              }}
                              placeholder="200 or ${variables.key}"
                              className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white text-sm"
                            />
                            <div className="mt-1 text-xs text-gray-400">
                              Retry until API response status code matches this value. Supports variable interpolation: ${'{variables.key}'} or ${'{data.key.path}'}
                            </div>
                          </>
                        );
                      })()}
